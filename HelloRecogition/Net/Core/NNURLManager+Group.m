...
@implementation NNURLManager (Group)
...
- (NSString *)sendGroupRequest:(nullable NNGroupRequestConfigBlock)configBlock
    ...
    NNGroupRequest *groupRequest = [[NNGroupRequest alloc] init];
    ...
    if (groupRequest.requestArray.count > 0) {
        ...
        for (NNRequest *request in groupRequest.requestArray) {
            
             NSString *taskID = [self sendRequest:request complete:^(NNResponse * _Nullable response) {
